...
#include "client/clientprocessor.h"
...
class AOClient : public QObject, public kal::PacketTransport
{
  Q_OBJECT
...
public:
   ...
   *
   * @param server A pointer to the Server instance where the client is joining to.
   * @param p_socket The socket associated with the AOClient.
   * @param playerId The user ID of the client.
   * @param parent Qt-based parent, passed along to inherited constructor from QObject.
   */
  AOClient(Server *server, kal::SocketClient *socket, const QHostAddress &remoteIp, const QString &ipid, kal::PlayerId playerId, MusicManager *musicManager = nullptr, QObject *parent = nullptr);
  ~AOClient();
  ...
  kal::AreaId areaId() const;
  void setAreaId(const kal::AreaId f_area_id);
   ...
   *
   * @see AOClient::kal::UpdateAreaPacket::Update
   */
  void arup(kal::UpdateAreaPacket::Update type, bool broadcast);
   ...
   */
  kal::CharacterId m_char_id = kal::NoCharacterId;
   ...
   */
  void handlePacket(const kal::Packet &packet);
...
public Q_SLOTS:
   ...
   */
  void shipPacket(const kal::Packet &packet);
...
private:
  ...
  kal::ClientHandshakeProcessor m_handshake_processor;
  kal::ClientSessionProcessor m_session_processor;
  kal::ClientProcessor *m_processor = &m_handshake_processor;

  /**
   * @brief The ID of the area the client is currently in.
   */
  kal::AreaId m_current_area = 0;
   ...
   * @return IC-Message stored in the QList.
   */
