...
AreaData::AreaData(Server *server, QString p_name, int p_index, MusicManager *p_music_manager = nullptr)
    : m_server(server)
    , m_index(p_index)
...
void AreaData::addClient(int f_charId, int f_userId)
{
  ...
  m_server->unicast(kal::PlayTrackPacket{m_currentAmbience, kal::NoCharacterId, ConfigManager::serverName()}, f_userId);
  m_server->unicast(kal::PlayTrackPacket{m_currentMusic, kal::NoCharacterId, ConfigManager::serverName()}, f_userId);
}
...
QList<kal::Evidence> AreaData::evidence() const
{
  return m_evidence;
}
...
void AreaData::appendEvidence(const kal::Evidence &f_evi_r)
{
...
void AreaData::replaceEvidence(int f_eviId, const kal::Evidence &f_newEvi_r)
{
...
const QList<QStringList> &AreaData::testimony() const
{
  return m_testimony;
}
...
void AreaData::changeHP(kal::HealthType f_side, int f_newHP)
{
  switch (f_side)
  {
  default:
  ...
  case kal::DefenseHealth:
    m_defHP = std::clamp(f_newHP, 0, 10);
    break;

  case kal::ProsecutionHealth:
    m_proHP = std::clamp(f_newHP, 0, 10);
    break;
  }
}
...
std::optional<QString> AreaData::side() const
{
  return m_side;
}
...
QString AreaData::addJukeboxSong(QString f_song)
{
  if (!m_jukebox_queue.contains(f_song))
  {
    ...
    if (l_song.second > 0)
    {
      ...
      {
        m_server->broadcast(kal::PlayTrackPacket{l_song.first, kal::NoCharacterId}, m_index);
...
QList<int> AreaData::joinedIDs() const
{
  return m_joined_ids;
}
