...
MusicManager::MusicManager(Server *server, QStringList f_cdns, MusicList f_root_list, QStringList f_root_ordered, QObject *parent)
    : QObject(parent)
    , m_server(server)
...
{
  kal::MusicListPacket packet;
  packet.music_list = musiclist(f_area_index);
  m_server->unicast(packet, f_user_id);
}

void MusicManager::sendAreaFMPacket(int f_area_index)
{
  kal::MusicListPacket packet;
  packet.music_list = musiclist(f_area_index);
  m_server->broadcast(packet, f_area_index);
}
