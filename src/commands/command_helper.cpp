...
{
  AreaData *l_area = m_server->area(area_idx);
  QTimer *l_timer;
...
{
  QStringList entries;
  QString area_name = m_server->getAreaName(area_idx);
  AreaData *area = m_server->area(area_idx);
  ...
  for (AOClient *l_client : l_clients)
  {
    ...
    {
      QString char_entry = "[" + QString::number(l_client->playerId()) + "] " + l_client->character();
...
void AOClient::sendNotice(QString f_notice, bool f_global)
{
  ...
  kal::NoticePacket l_packet(l_message);
  if (f_global)
  {
