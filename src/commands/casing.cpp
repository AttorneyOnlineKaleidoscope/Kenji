...
void AOClient::cmdExamine(int argc, QStringList argv)
{
  ...
  if (l_area->testimony().size() - 1 > 0)
  {
    ...
    m_server->broadcast(kal::SplashPacket(kal::SplashPacket::CrossExamination), areaId());

    // TODO implement MS packet
...
void AOClient::cmdUpdateStatement(int argc, QStringList argv)
{
  ...
  m_server->area(areaId())->setTestimonyRecording(AreaData::TestimonyRecording::UPDATE);
...
void AOClient::cmdPauseTestimony(int argc, QStringList argv)
{
  ...
  m_server->broadcast(kal::SplashPacket(kal::SplashPacket::WitnessTestimony), areaId());
...
void AOClient::cmdAddStatement(int argc, QStringList argv)
{
  ...
  if (m_server->area(areaId())->statement() < ConfigManager::maxStatements())
  {
    m_server->area(areaId())->setTestimonyRecording(AreaData::TestimonyRecording::ADD);
