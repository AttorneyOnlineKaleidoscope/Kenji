...
public:
   ...
   */
  AreaData(Server *server, QString p_name, int p_index, MusicManager *p_music_manager);
   ...
   * @see #m_evidence
   */
  QList<kal::Evidence> evidence() const;
   ...
   */
  void appendEvidence(const kal::Evidence &f_evi_r);
   ...
   */
  void replaceEvidence(int f_eviId, const kal::Evidence &f_newEvi_r);
   ...
   */
  void changeHP(kal::HealthType f_side, int f_newHP);
   ...
   */
  const QList<QStringList> &testimony() const;
...
private:
  ...
  /**
   * @brief A list of kal::Evidence currently available in the area's court record.
   *
   * @details This contains *all* evidence, not just the ones a given side can see.
   *
   * @see HIDDEN_CM
   */
  QList<kal::Evidence> m_evidence;
  ...
  QList<QStringList> m_testimony; //!< Vector of all statements saved. Index 0 is always the title of the testimony.
